...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    // 1. Logika Khusus untuk Hardcoded Admin
    if (username.toLowerCase() === 'admin' && password === 'admin') {
      try {
        // Simpan tanda/token admin jika diperlukan di local storage
        localStorage.setItem('nexabi_token', 'mock-admin-token-2026');
        localStorage.setItem('nexabi_username', 'admin');
        localStorage.setItem('user_role', 'admin');

        setLoading(false);
        // Menuju rute dashboard utama (menggunakan DashboardLayout.jsx)
        return navigate('/dashboard', { replace: true });
      } catch (err) {
        setError('Gagal memproses login admin.');
        setLoading(false);
        return;
      }
    }

    // 2. Logika untuk Customer / Pengguna Umum via API FastAPI
    try {
      ...
      // Simpan data token hasil response API ke localStorage
      localStorage.setItem('nexabi_token', res.data.access_token);
      localStorage.setItem('nexabi_username', username);
      localStorage.setItem('user_role', 'customer');

      // Menuju rute dashboard customer (menggunakan DashboardLayoutcustomer.jsx)
