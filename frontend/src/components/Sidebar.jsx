...
// 2. Menu Khusus untuk Customer (Silakan sesuaikan rute dan labelnya)
const customerNavItems = [
  { to: '/customer-dashboard', icon: LayoutDashboard, label: 'Overview', end: true },
  { to: '/customer-dashboard/analytics', icon: TrendingUp, label: 'My Analytics' },
];

export default function Sidebar() {
  const navigate = useNavigate();

  // 3. Ambil role user dari localStorage untuk menentukan menu mana yang muncul
  ...
  return (
    <aside className="flex flex-col w-60 flex-shrink-0 border-r"
      ...
      <nav className="flex-1 px-3 py-4 space-y-1">
        <p className="text-muted text-xs font-semibold uppercase tracking-wider px-2 mb-3">
          Menu {role === 'admin' ? 'Admin' : 'Customer'}
        </p>

        {navItems.map(({ to, icon: Icon, label, end }) => (
          <NavLink key={to} to={to} end={end}
            className={({ isActive }) =>
              `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${isActive ? 'text-white' : 'text-muted hover:text-white hover:bg-white/5'
              }`
            }
